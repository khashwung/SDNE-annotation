...
class Graph(object):
    def __init__(self, file_path, ng_sample_ratio):
        ...
        if suffix == "txt":
            ...
            # dok_matrix是稀疏矩阵，用于incrementally construct，效率很高
            ...
            for line in fin.readlines():
                ...
                count += 1
            fin.close()
            # 将稀疏矩阵转换为compressed sparse row format
    ...
    def load_label_data(self, filename):
        with open(filename,"r") as fin:
            firstLine = fin.readline().strip().split()
            # label的种类，根据label文件可以看到，这是一个multilabel问题
    ...
    def sample(self, batch_size, do_shuffle = True, with_label = False):
        ...
        # 获取采样的部分的邻接矩阵，行数取index中的部分，列数也要取index中的部分
